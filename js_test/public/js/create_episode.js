...
var thumbnailUrl =
  "http://34.145.65.5:46351/thumbnail_archive/" + encodeURIComponent(title);
var thumbnailImg = []; // 썸네일 저장 배열
...
fetch(thumbnailUrl)
  .then(function (response) {
    if (response.ok) {
      ...
      throw new Error("Failed to fetch thumbnail zip file");
    }
  })
  .then(function (blob) {
    ...
    zip.forEach(function (relativePath, zipEntry) {
      if (!zipEntry.dir && zipEntry.name.endsWith(".png")) {
        var promise = zipEntry.async("blob").then(function (content) {
          // Create a new object to store the file information
          var pngFile = {
            name: zipEntry.name,
            blob: content,
          };
          ...
          // Add the PNG file object to the array
          thumbnailImg.push(pngFile);
        });
      }
    });
  })
      ...
      zip.forEach(function (relativePath, zipEntry) {
        if (!zipEntry.dir && zipEntry.name.endsWith(".png")) {
          var promise = zipEntry.async("blob").then(function (content) {
            ...
            if (thumbnailImg[count] != null) {
              image.src = URL.createObjectURL(thumbnailImg[count].blob);
            } else {
              image.src = "/img/wt_cover.jpg"; // Default thumbnail image
            }
            image.alt = "Image" + (count + 1);
            image.style = "width: 106px; height: 62px";

            var webtoonImg = document.createElement("img");
            webtoonImg.src = URL.createObjectURL(content);
            webtoonImg.alt = "Image" + (count + 1);
            ...
            titleElement.textContent = title + " " + (count + 1) + "화";
              ...
              <body>
                ...
                <main>
                  ...
                  <div id="image-container">
                    <img src="${webtoonImg.src}" alt="${webtoonImg.alt}" />
                  </div>
                </main>
                <script>
                  // 회차 목록으로 돌아가기
            ...
            urls.push(webtoonImg.src); // Store image URL in the array
